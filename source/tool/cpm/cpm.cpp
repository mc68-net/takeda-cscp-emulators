...
{
	if(argc < 2) {
		printf("CP/M Player for Win32 console\n\n");
		printf("Usage: CPM (command file) [opions]\n");
		return;
	}
	...
	// init cpp
	mem[CPP_BASE + 0x00] = 0xed; mem[CPP_BASE + 0x01] = 0xfe;
	
	// init bdos
	mem[BDOS_BASE + 0x06] = 0xed; mem[BDOS_BASE + 0x07] = 0xff; mem[BDOS_BASE + 0x08] = 0xc9;
	
	// init bios
	mem[BIOS_BASE + 0x00] = 0xed; mem[BIOS_BASE + 0x01] = 0xe0; mem[BIOS_BASE + 0x02] = 0xc9;
	mem[BIOS_BASE + 0x03] = 0xed; mem[BIOS_BASE + 0x04] = 0xe1; mem[BIOS_BASE + 0x05] = 0xc9;
	mem[BIOS_BASE + 0x06] = 0xed; mem[BIOS_BASE + 0x07] = 0xe2; mem[BIOS_BASE + 0x08] = 0xc9;
	mem[BIOS_BASE + 0x09] = 0xed; mem[BIOS_BASE + 0x0a] = 0xe3; mem[BIOS_BASE + 0x0b] = 0xc9;
	mem[BIOS_BASE + 0x0c] = 0xed; mem[BIOS_BASE + 0x0d] = 0xe4; mem[BIOS_BASE + 0x0e] = 0xc9;
	mem[BIOS_BASE + 0x0f] = 0xed; mem[BIOS_BASE + 0x10] = 0xe5; mem[BIOS_BASE + 0x11] = 0xc9;
	mem[BIOS_BASE + 0x12] = 0xed; mem[BIOS_BASE + 0x13] = 0xe6; mem[BIOS_BASE + 0x14] = 0xc9;
	mem[BIOS_BASE + 0x15] = 0xed; mem[BIOS_BASE + 0x16] = 0xe7; mem[BIOS_BASE + 0x17] = 0xc9;
	mem[BIOS_BASE + 0x18] = 0xed; mem[BIOS_BASE + 0x19] = 0xe8; mem[BIOS_BASE + 0x1a] = 0xc9;
	mem[BIOS_BASE + 0x1b] = 0xed; mem[BIOS_BASE + 0x1c] = 0xe9; mem[BIOS_BASE + 0x1d] = 0xc9;
	mem[BIOS_BASE + 0x1e] = 0xed; mem[BIOS_BASE + 0x1f] = 0xea; mem[BIOS_BASE + 0x20] = 0xc9;
	mem[BIOS_BASE + 0x21] = 0xed; mem[BIOS_BASE + 0x22] = 0xeb; mem[BIOS_BASE + 0x23] = 0xc9;
	mem[BIOS_BASE + 0x24] = 0xed; mem[BIOS_BASE + 0x25] = 0xec; mem[BIOS_BASE + 0x26] = 0xc9;
	mem[BIOS_BASE + 0x27] = 0xed; mem[BIOS_BASE + 0x28] = 0xed; mem[BIOS_BASE + 0x29] = 0xc9;
	mem[BIOS_BASE + 0x2a] = 0xed; mem[BIOS_BASE + 0x2b] = 0xee; mem[BIOS_BASE + 0x2c] = 0xc9;
	mem[BIOS_BASE + 0x2d] = 0xed; mem[BIOS_BASE + 0x2e] = 0xef; mem[BIOS_BASE + 0x2f] = 0xc9;
	mem[BIOS_BASE + 0x30] = 0xed; mem[BIOS_BASE + 0x31] = 0xf0; mem[BIOS_BASE + 0x32] = 0xc9;
	mem[BIOS_BASE + 0x33] = 0xed; mem[BIOS_BASE + 0x34] = 0xf1; mem[BIOS_BASE + 0x35] = 0xc9;
	mem[BIOS_BASE + 0x36] = 0xed; mem[BIOS_BASE + 0x37] = 0xf2; mem[BIOS_BASE + 0x38] = 0xc9;
	mem[BIOS_BASE + 0x39] = 0xed; mem[BIOS_BASE + 0x3a] = 0xf3; mem[BIOS_BASE + 0x3b] = 0xc9;
	mem[BIOS_BASE + 0x3c] = 0xed; mem[BIOS_BASE + 0x3d] = 0xf4; mem[BIOS_BASE + 0x3e] = 0xc9;
	mem[BIOS_BASE + 0x3f] = 0xed; mem[BIOS_BASE + 0x40] = 0xf5; mem[BIOS_BASE + 0x41] = 0xc9;
	mem[BIOS_BASE + 0x42] = 0xed; mem[BIOS_BASE + 0x43] = 0xf6; mem[BIOS_BASE + 0x44] = 0xc9;
	mem[BIOS_BASE + 0x45] = 0xed; mem[BIOS_BASE + 0x46] = 0xf7; mem[BIOS_BASE + 0x47] = 0xc9;
	mem[BIOS_BASE + 0x48] = 0xed; mem[BIOS_BASE + 0x49] = 0xf8; mem[BIOS_BASE + 0x4a] = 0xc9;
...
void bdos()
{
	...
	case 9:
		// output string
		for(int i = 0; i < 256; i++) {
			if((string[i] = mem[DE + i]) == delimiter) {
				string[i] = '\0';
				break;
			}
		}
	...
	case 15:
		...
		if((drive = mem[DE] ? (mem[DE] - 1) : default_drive) < MAX_DRIVES) {
			login_drive |= 1 << drive;
			CreatePath(drive, mem + DE + 1, path);
	...
	case 17:
		...
		if((drive = mem[DE] ? (mem[DE] - 1) : default_drive) < MAX_DRIVES) {
			...
			if(find_num > 0) {
				CreatePath(find_files[find_idx][11], find_files[find_idx], path);
				if((size = GetFileSize(path)) >= 0) {
	...
	case 20:
		...
		if((drive = mem[DE] ? (mem[DE] - 1) : default_drive) < MAX_DRIVES) {
			...
			record += mem[DE + 14] << 20;
			CreatePath(drive, mem + DE + 1, path);
			if((fp = GetFilePtr(path)) != NULL) {
				if(fseek(fp, record * 128, SEEK_SET) == 0) {
					if(fread(mem + dma_addr, 128, 1, fp) != 0) {
						record++;
	...
	case 23:
		...
		if((drive = mem[DE] ? (mem[DE] - 1) : default_drive) < MAX_DRIVES) {
			if(!read_only[drive]) {
				login_drive |= 1 << drive;
				CreatePath(drive, mem + DE + 1, path);
				CreatePath(drive, mem + DE + 17, string);
				if((size = GetFileSize(path)) >= 0) {
					int ex = (mem[DE + 12] & 0x1f) | (mem[DE + 13] << 5);
					int block = size - ex * 16384;
					for(int i = 0; i < 16; i++)
...
void CreatePath(int drive, uint8* src, char* dest)
{
	char file[9], ext[4];
	for(int i = 0; i < 8; i++)
