...
void EMU::initialize_sound(int rate, int samples)
{
	...
	sound_ok = now_mute = now_rec = false;
	...
	// direct sound is successfully initialized
	sound_ok = true;
}
...
void EMU::start_rec_sound()
{
	if(!now_rec) {
		...
		if(rec->Fopen(file_path, FILEIO_WRITE_BINARY)) {
			...
			ZeroMemory(&header, sizeof(wavheader_t));
...
void EMU::stop_rec_sound()
{
	if(now_rec) {
		...
		header.dwFileSize = rec_size + sizeof(wavheader_t) - 8;
		...
		header.dwAvgBytesPerSec = sound_rate * 2;
		header.wBlockAlign = 2;
		header.wBitsPerSample = 16;
		header.dwdata = 0x61746164;
		header.dwDataLength = rec_size;
		
		rec->Fseek(0, SEEK_SET);
		rec->Fwrite(&header, sizeof(wavheader_t), 1);
		rec->Fclose();
		delete rec;
		now_rec = false;
