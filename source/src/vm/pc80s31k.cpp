...
void PC80S31K::initialize()
{
	...
	_stprintf(file_path, _T("%sDISK.ROM"), app_path);
...
uint32 PC80S31K::read_io8(uint32 addr)
{
	...
	case 0xfe:
		val = d_pio->read_io8(2);
		{
			...
			if(prev != val){
//				emu->out_debug("SUB\tIN DAV=%d,RFD=%d,DAC=%d,ATN=%d\n", val&1, (val>>1)&1, (val>>2)&1, (val>>3)&1);
				prev = val;
			}
		}
		return val;
...
//		emu->out_debug("SUB\tOUT DAV=%d,RFD=%d,DAC=%d,ATN=%d\n", (data>>4)&1, (data>>5)&1, (data>>6)&1, (data>>7)&1);
	...
	case 0xff:
		if(!(data & 0x80)) {
			int bit = (data >> 1) & 7;
			if(bit == 4){
//				emu->out_debug("SUB\tOUT DAV=%d\n", data & 1);
			}
			else if(bit == 5){
//				emu->out_debug("SUB\tOUT RFD=%d\n", data & 1);
			}
			else if(bit == 6){
//				emu->out_debug("SUB\tOUT DAC=%d\n", data & 1);
			}
			else if(bit == 7){
