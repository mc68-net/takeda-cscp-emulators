...
OP( 0x9a, i_call_far  ) {
	...
	if(nec_state->bios != NULL && nec_state->bios->bios_call_i86(((tmp2 << 4) + tmp) & 0xfffff, nec_state->regs.w, nec_state->sregs, &ZeroVal, &CarryVal)) {
		...
		nec_state->CarryVal = CarryVal;
		CLKW(29,29,13,29,21,9,Wreg(SP));
		return;
	}
#endif
	PUSH(Sreg(PS)); PUSH(nec_state->ip); nec_state->ip = (WORD)tmp; Sreg(PS) = (WORD)tmp2; CHANGE_PC; CLKW(29,29,13,29,21,9,Wreg(SP));
}
...
#endif
	nec_interrupt(nec_state, int_num, BRK); CLKS(50,50,24);
}
...
#endif
	PUSH(nec_state->ip); nec_state->ip = (WORD)(nec_state->ip+(INT16)tmp); CHANGE_PC; nec_state->icount-=24;
}
...
OP( 0xff, i_ffpre ) { UINT32 tmp, tmp1, tmp2; GetModRM; tmp=GetRMWord(ModRM);
	switch(ModRM & 0x38) {
		...
		case 0x10:
#ifdef V30_BIOS_CALL
			{
				...
				if(nec_state->bios != NULL && nec_state->bios->bios_call_i86(((Sreg(PS) << 4) + tmp) & 0xfffff, nec_state->regs.w, nec_state->sregs, &ZeroVal, &CarryVal)) {
					nec_state->ZeroVal = ZeroVal;
					nec_state->CarryVal = CarryVal;
					nec_state->icount -= (ModRM >= 0xc0) ? 16 : 20;
					return;
				}
			}
#endif
		...
		case 0x18:
...
#ifdef V30_BIOS_CALL
			{
				...
				if(nec_state->bios != NULL && nec_state->bios->bios_call_i86(((tmp2 << 4) + tmp) & 0xfffff, nec_state->regs.w, nec_state->sregs, &ZeroVal, &CarryVal)) {
					nec_state->ZeroVal = ZeroVal;
					nec_state->CarryVal = CarryVal;
					nec_state->icount -= (ModRM >= 0xc0) ? 16 : 26;
					return;
				}
			}
#endif
			tmp1 = Sreg(PS); Sreg(PS) = tmp2; PUSH(tmp1); PUSH(nec_state->ip); nec_state->ip = tmp; CHANGE_PC; nec_state->icount-=(ModRM >=0xc0 )?16:26; break; /* CALL FAR */
