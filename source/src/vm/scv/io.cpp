...
uint32 IO::read_io8(uint32 addr)
{
	...
	case P_B:
		if(!(pa & 0x01)) {
			if((joy[0] & 0x04) || key[0x25]) val &= ~0x01;	// P1-L
			if((joy[0] & 0x01) || key[0x26]) val &= ~0x02;	// P1-U
			if((joy[0] & 0x10) || key[0x5a]) val &= ~0x04;	// P1-TR1
			if(joy[1] & 0x04               ) val &= ~0x08;	// P2-L
			if(joy[1] & 0x01               ) val &= ~0x10;	// P2-U
			if(joy[1] & 0x10               ) val &= ~0x20;	// P2-TR1
		}
		if(!(pa & 0x02)) {
			if((joy[0] & 0x02) || key[0x28]) val &= ~0x01;	// P1-D
			if((joy[0] & 0x08) || key[0x27]) val &= ~0x02;	// P1-R
			if((joy[0] & 0x20) || key[0x58]) val &= ~0x04;	// P1-TR2
