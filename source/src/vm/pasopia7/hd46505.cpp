...
void HD46505::initialize()
{
...
#ifdef _LCD
	for(int i = 1; i < 8; i++)
		palette_pc[i] = RGB_COLOR(6, 7, 2);
	palette_pc[0] = RGB_COLOR(20, 21, 20);
#else
	for(int i = 0; i < 8; i++)
	...
	mode = text_page = 0;
	cblink = flash_cnt = 0;
	hsync = vsync = display = blink = pal_dis = false;
	
	// display type
#ifdef _LCD
	dev->write_signal(dev_id, 0, 0x10);
#else
	dev->write_signal(dev_id, 0xffffffff, 0x10);
#endif
...
uint32 HD46505::read_io8(uint32 addr)
{
	...
	case 0x10:
		return ch;
	case 0x11:
		return (12 <= ch && ch <= 17) ? regs[ch] : 0xff;
	}
	return 0xff;
}

void HD46505::write_signal(int id, uint32 data, uint32 mask)
{
	if(id == SIG_HD46505_I8255_0_A)
		mode = data;
	else if(id == SIG_HD46505_I8255_1_B)
		text_page = (data >> 4) & 7;
	else if(id == SIG_HD46505_I8255_1_C) {
...
void HD46505::event_vsync(int v, int clock)
{
	...
	if(vsync != sync) {
		dev->write_signal(dev_id, sync ? 0xffffffff : 0, 0x20);
		vsync = sync;
	}
}

void HD46505::event_hsync(int v, int h, int clock)
{
	bool sync = (hs <= h && h <= he) ? true : false;
	bool disp = (h < dhe && v < dve) ? true : false;
	if(hsync != sync) {
		hsync = sync;
	}
	if(display != disp) {
		dev->write_signal(dev_id, disp ? 0xffffffff : 0, 8);
		display = disp;
	}
}

void HD46505::draw_screen()
{
#ifdef _LCD
...
void HD46505::draw_text_normal(uint16 src)
{
...
void HD46505::draw_text_wide(uint16 src)
{
...
void HD46505::draw_fine_normal(uint16 src)
{
...
void HD46505::draw_fine_wide(uint16 src)
{
...
void HD46505::draw_text_lcd(uint16 src)
{
...
void HD46505::draw_fine_lcd(uint16 src)
{
	// fine graph mode, normal char (80chars)
	for(int y = 0, yy = 0; y < 64; y += 8, yy++) {
		int src_g = (src + 640 * (yy & 3) + (yy & 4 ? 8 : 0)) & 0x3ff8;
