...
#define RDOP(O)     O = cpustate->program->read_data8(PCD); PC++
#define RDOPARG(A)  A = cpustate->program->read_data8(PCD); PC++
#define RM(A)       cpustate->program->read_data8(A)
#define WM(A,V)     cpustate->program->write_data8(A,V)
...
static UINT8 RP(upd7810_state *cpustate, offs_t port)
{
	...
	case UPD7810_PORTA:
		if (cpustate->ma)   // NS20031301 no need to read if the port is set as output
			cpustate->pa_in = cpustate->io->read_io8(port);
	...
	case UPD7810_PORTB:
		if (cpustate->mb)   // NS20031301 no need to read if the port is set as output
			cpustate->pb_in = cpustate->io->read_io8(port);
	...
	case UPD7810_PORTC:
		if (cpustate->mc)   // NS20031301 no need to read if the port is set as output
			cpustate->pc_in = cpustate->io->read_io8(port);
		...
		break;
	case UPD7810_PORTD:
		cpustate->pd_in = cpustate->io->read_io8(port);
		...
		break;
	case UPD7810_PORTF:
		cpustate->pf_in = cpustate->io->read_io8(port);
	...
	case UPD7807_PORTT: // NS20031301 partial implementation
		data = cpustate->io->read_io8(port);
		break;
	default:
...
//      data = (data & ~cpustate->ma) | (cpustate->pa_in & cpustate->ma);
		...
		cpustate->io->write_io8(port, data);
		break;
	case UPD7810_PORTB:
...
{
	upd7810_state *cpustate = (upd7810_state *)calloc(1, sizeof(upd7810_state));
	return cpustate;
}
...
static int run_one_opecode(upd7810_state *cpustate)
{
	cpustate->icount = 0;
...
static CPU_EXECUTE( upd7810 )
{
	...
	if(now_debugging) {
		...
		icount = run_one_opecode(cpustate);
